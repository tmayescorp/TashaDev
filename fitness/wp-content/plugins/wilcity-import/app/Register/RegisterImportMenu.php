...
class RegisterImportMenu
{
    ...
    protected $aMenuCollection = ['Discover', 'Company', 'wilMenu'];
    ...
    public function __construct()
    {
        ...
        add_action('admin_init', [$this, 'redirectToSetup']);
        add_action('wp_ajax_wilcity_importing_demo', [$this, 'runSetup']);
    }
    ...
    private function verifyRequiredPlugins()
    {
        if (!defined('WILOKE_LISTING_TOOL_VERSION')) {
            wp_send_json_error(
              [
                'msg'        => 'In order to use Wilcity Import, please activate all required plugins first: Appearance -> Install Plugins',
                'done'       => true,
                'item_error' => true
              ]
            );
        ...
        if ($this->pageBuilder === 'elementor') {
            if (!defined('WILCITY_EL_VERSION') || !defined('ELEMENTOR_VERSION')) {
                wp_send_json_error(
                  [
                    'msg'        => 'In order to use Wilcity Import, please activate all required plugins, Elementor plugin and Wilcity Elementor plugin: Appearance -> Install Plugins',
                    'done'       => true,
                    'item_error' => true
                  ]
                );
        ...
        if ($this->pageBuilder === 'vc') {
            if (!defined('WILCITY_VC_SC') || !defined('WPB_PLUGIN_FILE')) {
                wp_send_json_error(
                  [
                    'msg'        => 'In order to use Wilcity Import, please activate all required plugins, Wilcity WP Pagebuilder plugin and WP Bakery Pagebuilder plugin: Appearance -> Install Plugins',
                    'done'       => true,
                    'item_error' => true
                  ]
                );
    ...
    private function searchPageByTemplate($templateName)
    {
        global $wpdb;
        $pageID = $wpdb->get_var(
          $wpdb->prepare("SELECT post_id from $wpdb->postmeta WHERE meta_value=%s AND meta_key='_wp_page_template'",
            $templateName)
        );
    ...
    private function getListingPlans()
    {
        global $wpdb;
        
        $aRawPlans = $wpdb->get_results(
          "SELECT ID FROM {$wpdb->posts} WHERE post_name IN ('basic', 'premium', 'advanced') AND post_type='listing_plan'",
          ARRAY_A
        );
        
        $aPlans = [];
        if (is_array($aRawPlans)) {
            foreach ($aRawPlans as $aPlan) {
                $aPlans[] = $aPlan['ID'];
            }
    ...
    {
        return $this->searchPageByTemplate('dashboard/index.php');
    }
    
    private function getPackagePage()
    {
        $pageID = $this->searchPageByTemplate('wiloke-submission/pricing.php');
        
        if (!empty($pageID)) {
            switch ($this->pageBuilder) {
                case 'elementor':
                    $pricingDir = $this->getElementorFile('add-listing-plan.json');
                    
                    if ($pricingDir) {
                        $instWilcityELImport = new WilcityElemtorImport();
                        $instWilcityELImport->run($pricingDir, $pageID);
                    }
                    
                    break;
                case 'vc':
                    $pageID = $this->getPostBySlug('add-listing-plan-3');
                    break;
            }
        }
    ...
    private function getAddListingPage()
    {
        return $this->searchPageByTemplate('wiloke-submission/addlisting.php');
    }
    
    private function getCheckoutPage()
    {
        return $this->searchPageByTemplate('wiloke-submission/checkout.php');
    }
    
    private function getThankyouPage()
    {
        return $this->searchPageByTemplate('wiloke-submission/thankyou.php');
    }
    
    private function getCancelPage()
    {
        return $this->searchPageByTemplate('wiloke-submission/cancel.php');
    }
    
    private function importWilokeSubmission()
    {
        ...
        foreach ($aConfigs as $aPage) {
            ...
            if (empty($postID)) {
                $postID = wp_insert_post([
                  ...
                  'post_content' => $this->pageBuilder == 'kc' ? $aPage['content'] : '',
                  'post_status'  => 'publish',
                  'post_type'    => 'page'
                ]);
        ...
        if ($hasUpdated) {
            $aConfigs                          = wilokeListingToolsRepository()
              ->get('wiloke-submission:configuration', true)
              ->sub('fields')
            ;
            $aConfigs['become_an_author_page'] = $this->getPostBySlug('become-an-author');
            ...
            $aPlans                                       = $this->getListingPlans();
            $aWilokeSubmission['listing_plans']           = implode(',', $aPlans);
            $aWilokeSubmission['free_claim_listing_plan'] = $aPlans[0];
            $aWilokeSubmission['toggle_become_an_author'] = 'enable';
            $aWilokeSubmission['dashboard_page']          = $this->getDashboardPage();
            $aWilokeSubmission['package']                 = $this->getPackagePage();
            $aWilokeSubmission['addlisting']              = $this->getAddListingPage();
            $aWilokeSubmission['thankyou']                = $this->getThankyouPage();
            $aWilokeSubmission['cancel']                  = $this->getCancelPage();
            update_option('wiloke_submission_configuration', maybe_serialize($aWilokeSubmission));
        }
    }
    
    private function importSearchForm()
    {
        ...
        foreach ($aPostTypeKeys as $postType) {
            $content  =
              file_get_contents(WILCITY_IMPORT_DIR.$this->importDir.$this->ds.'search-form/'.$postType.'.json');
            $aContent = json_decode($content, true);
            SetSettings::setOptions(General::getSearchFieldsKey($postType), $aContent['settings']['settings']);
        }
    }
    
    private function importHeroSearchForm()
    {
        $aPostTypeKeys = General::getPostTypeKeys(false, false);
        if (empty($aPostTypeKeys)) {
            return false;
        ...
        foreach ($aPostTypeKeys as $postType) {
            $content  =
              file_get_contents(WILCITY_IMPORT_DIR.$this->importDir.$this->ds.'hero-search-form/'.$postType.'.json');
            $aContent = json_decode($content, true);
            SetSettings::setOptions(General::getHeroSearchFieldsKey($postType), $aContent['settings']['settings']);
        }
    }
    
    private function importSingleSidebar()
    {
        $aPostTypeKeys = General::getPostTypeKeys(false, false);
        if (empty($aPostTypeKeys)) {
            return false;
        ...
        foreach ($aPostTypeKeys as $postType) {
            $content  =
              file_get_contents(WILCITY_IMPORT_DIR.$this->importDir.$this->ds.'single-sidebar/'.$postType.'.json');
            $aContent = json_decode($content, true);
            SetSettings::setOptions(General::getSingleListingSettingKey('sidebar', $postType),
              $aContent['settings']['settings']);
        }
    }
    
    private function importSingleNavigation()
    {
        $aPostTypeKeys = General::getPostTypeKeys(false, false);
        if (empty($aPostTypeKeys)) {
            return false;
        ...
        foreach ($aPostTypeKeys as $postType) {
            $content  =
              file_get_contents(WILCITY_IMPORT_DIR.$this->importDir.$this->ds.'single-navigation/'.$postType.'.json');
            $aContent = json_decode($content, true);
    ...
    private function importSingleHighlightBoxes()
    {
        $aPostTypeKeys = General::getPostTypeKeys(false, false);
        if (empty($aPostTypeKeys)) {
            return false;
        ...
        foreach ($aPostTypeKeys as $postType) {
            $content  =
              file_get_contents(WILCITY_IMPORT_DIR.$this->importDir.$this->ds.'single-highlight-boxes/'.$postType.
                                '.json');
            $aContent = json_decode($content, true);
    ...
    private function findSearchV2ID()
    {
        return $this->searchPageByTemplate('templates/search-v2.php');
    }
    
    private function importListingCard()
    {
        $aPostTypeKeys = General::getPostTypeKeys(false, false);
        if (empty($aPostTypeKeys)) {
            return false;
        ...
        foreach ($aPostTypeKeys as $postType) {
            $content = file_get_contents(
              WILCITY_IMPORT_DIR.$this->importDir.$this->ds.'listing-card/'.$postType.'.json'
            );
            
            $aContent = json_decode($content, true);
            
            SetSettings::setOptions(
              General::getSingleListingSettingKey('card', $postType),
              $aContent['settings']['aBodyUsedFields']
            );
            
            SetSettings::setOptions(
              General::getSingleListingSettingKey('footer_card', $postType),
              $aContent['settings']['aFooterSettings']
            );
            
            SetSettings::setOptions(
              General::getSingleListingSettingKey('header_card', $postType),
              $aContent['settings']['aHeaderSettings']
            );
        }
    }
    
    private function importDirectoryTypes()
    {
        $content  =
          file_get_contents(WILCITY_IMPORT_DIR.$this->importDir.$this->ds.'directory-types/directory-types.json');
        $aContent = json_decode($content, true);
        SetSettings::setOptions(wilokeListingToolsRepository()->get('addlisting:customPostTypesKey'),
          $aContent['settings']['settings']);
    }
    
    private function importAddListing()
    {
        $aPostTypeKeys = General::getPostTypeKeys(false, false);
        if (empty($aPostTypeKeys)) {
            return false;
        ...
        foreach ($aPostTypeKeys as $postType) {
            $content  =
              file_get_contents(WILCITY_IMPORT_DIR.$this->importDir.$this->ds.'addlisting/'.$postType.'.json');
            $aContent = json_decode($content, true);
            SetSettings::setOptions(General::getUsedSectionKey($postType), $aContent['settings']);
        }
    }
    ...
    public function canUnZip()
    {
        ...
        $fields   = ['action', '_wp_http_referer', '_wpnonce'];
        $canUnZip = false;
        if (false !== ($creds = request_filesystem_credentials('', '', false, false, $fields))) {
            
            if (!WP_Filesystem($creds)) {
                request_filesystem_credentials(wp_nonce_url(admin_url('admin.php?page='.$this->slug)), '', true, false,
    ...
    {
        return !$isSuccess ?
          sprintf(esc_html__('We could not found %s plugin. Maybe the plugin has been removed'), $name) :
          sprintf(esc_html__('%s plugin has been installed'), $name);
    }
        ...
        foreach ($data as $sidebar_id => $widgets) {
            ...
            foreach ($widgets as $widget_instance_id => $widget) {
                ...
                if (!$fail && !isset($available_widgets[$id_base])) {
                    ...
                    $widget_message      =
                      esc_html__('Site does not support widget', 'wiloke'); // explain why widget not imported
                }
                ...
                if (!$fail && isset($widget_instances[$id_base])) {
                    ...
                    foreach ($single_widget_instances as $check_id => $check_widget) {
                        ...
                        if (in_array("$id_base-$check_id", $sidebar_widgets) && (array)$widget == $check_widget) {
                            ...
                            $widget_message      =
                              esc_html__('Widget already exists', 'wiloke'); // explain why widget not imported
                            break;
                        }
                    }
                ...
                $results[$sidebar_id]['widgets'][$widget_instance_id]['name']         =
                  isset($available_widgets[$id_base]['name']) ? $available_widgets[$id_base]['name'] :
                ...
                $results[$sidebar_id]['widgets'][$widget_instance_id]['title']        =
                  !empty($widget['title']) ? $widget['title'] :
                    esc_html__('No Title', 'wiloke'); // show "No Title" if widget instance is untitled
        ...
        foreach ($aData as $taxonomyName => $aInfo) {
            foreach ($aInfo as $parentSlug => $aChildrenSlug) {
                ...
                foreach ($aChildrenSlug as $childSlug) {
                    ...
                    wp_update_term($oChildInfo->term_id, $taxonomyName, [
    ...
    protected function setupMenu()
    {
        ...
        set_theme_mod('nav_menu_locations', [$this->menuLocation => $termID]);
    }
    
    protected function createSearchV2()
    {
        $query = new \WP_Query(
          [
            'post_type'      => 'page',
            ...
            'meta_value'     => 'templates/search-v2.php'
          ]
        );
        ...
        if (!is_wp_error($pageID)) {
            update_post_meta($pageID, 'page_template', 'templates/search-v2.php');
    ...
    protected function setFrontPage()
    {
        ...
        $aPosts =
          $wpdb->get_results("SELECT ID, post_content FROM $wpdb->posts WHERE post_type='page' AND post_status='publish' AND post_name LIKE '%".
                             $postName."%' ORDER BY ID DESC LIMIT 20");
        ...
        if (!empty($aPosts)) {
            foreach ($aPosts as $oPost) {
                if ($this->pageBuilder === 'elementor') {
                    if (get_post_meta($oPost->ID, '_elementor_edit_mode', true) === 'builder') {
                        return $oPost->ID;
                    }
                } else {
    ...
    private function getElementorFile($file)
    {
        $dummyDir = WILCITY_IMPORT_DIR.$this->importDir.$this->ds.'elementor'.$this->ds;
        
        return file_exists($dummyDir.$file) ? $dummyDir.$file : false;
    }
    ...
    private function importElementor()
    {
        ...
        $aELInstalled        = empty($aELInstalled) ? [] : $aELInstalled;
        $instWilcityELImport = new WilcityElemtorImport();
        ...
        foreach ($aELFiles as $file) {
            if (empty($aELInstalled) || !in_array($file, $aELInstalled)) {
                ...
                $msg        =
                  $status ? sprintf('%s has been imported.', $fileName) : sprintf('Could not import %s', $fileName);
    ...
    private function importVC()
    {
        ...
        $aVCInstalled = empty($aVCInstalled) ? [] : $aVCInstalled;
    ...
    private function importKC()
    {
        ...
        $aKCInstalled = empty($aKCInstalled) ? [] : $aKCInstalled;
    ...
    private function importLatLng()
    {
        $data  = file_get_contents(WILCITY_IMPORT_DIR.'dummy/special-tbls/latlng.json');
        $aData = json_decode($data, true);
        
        $query = new \WP_Query(
          [
            'post_type'      => ['listing', 'restaurant', 'renthouse'],
            'posts_per_page' => -1,
            'post_status'    => 'publish'
          ]
        );
        ...
        while ($query->have_posts()) {
            ...
            $aLatLng             = $aData[$order];
            $aLatLng['objectID'] = $query->post->ID;
            
            Listing::saveData($query->post->ID, $aLatLng);
            $order++;
        }
    }
    ...
    private function importHeroSearchFormSettings()
    {
        ...
        foreach ($aData as $postType => $aSettings) {
            SetSettings::setOptions(General::getHeroSearchFieldsKey($postType), $aSettings['settings']);
        }
        
        return true;
    }
    
    private function setPermalinkOption()
    {
    ...
    protected function cleanMenu()
    {
        ...
        $aTermIDs    = $wpdb->get_results(
          $wpdb->prepare(
            "SELECT $termsTbl.term_id FROM $termsTbl INNER JOIN $taxonomyTbl ON ($termsTbl.term_id = $taxonomyTbl.term_id) WHERE $termsTbl.name IN ('".
            implode('\',\'', $this->aMenuCollection)."') AND $taxonomyTbl.taxonomy=%s",
            'nav_menu'
          ),
          ARRAY_A
        );
    ...
    private function importThemeOptions()
    {
        $dummyDir        = WILCITY_IMPORT_DIR.$this->importDir.$this->ds;
        ...
        $aThemeOptions['search_page'] = $this->createSearchV2();
    ...
    public function runSetup()
    {
        try {
            ...
            if (count($aXML) === count($aDemoInstalled)) {
                                        ...
                                        foreach ($aTaxData['data'] as $termSlug => $aRawTermOptions) {
                                            foreach ($aRawTermOptions as $oTermMetaOptions) {
                                                ...
                                                if (!empty($oTerm) && !is_wp_error($oTerm)) {
                                                    update_term_meta($oTerm->term_id, $oTermMetaOptions['meta_key'],
                                                      $oTermMetaOptions['meta_value']);
                                                }
                                            }
                                        }
                                    }
                ...
                $this->importHeroSearchForm();
                $this->importSingleSidebar();
                $this->importSingleNavigation();
                $this->importListingCard();
                $this->importSearchForm();
                $this->importAddListing();
                $this->importWilokeSubmission();
                $this->importLatLng();
                $this->setPermalinkOption();
                $this->importThemeOptions();
                ...
                foreach ($aXML as $file) {
                    if (empty($aDemoInstalled) || !in_array($file, $aDemoInstalled)) {
                        ...
                        $fileName = end($aParseFile);
                        $msg      = $status ? sprintf('%s has been imported', $fileName) :
                          sprintf('Could not import %s', $fileName);
    ...
    {
        add_menu_page('Wilcity Import', 'Wilcity Import', 'administrator', $this->slug, [$this, 'setupArea'],
          'dashicons-nametag', 100);
    }
        ...
        <div id="wiloke-submission-wrapper">
            <div class="form ui">
                ...
                <div class="ui message info">
                    <p>First of all, thanks for using Wiloke Theme! The below are some useful information that explain
                        how to setup the theme and how to set up the theme's elements. If You have any beyond question,
                        don't hesitate to open a ticket at - <a href="http://help.wilcity.com/" target="_blank">www.help.wilcity.com
                    </p>
                    <p>
                        Don't forget to follow our fan page. We announce Wiloke's news via
                        <a href="https://www.facebook.com/wilokewp/" target="_blank">Facebook chanel</a> and <a
                                href="https://twitter.com/wilokethemes" target="_blank">Twitter chanel</a>
                    </p>
                </div>
            </div>
            ...
            <div class="ui message danger">
                <p>
                    Wilcity is compatible with 3 Page Builders: King Composer, Elementor, WPBakery Page Builder, You can
                    which one that is family with you. <span style="color: red;">If you select 1 Page Builder, we recommend disabling the rest. For example, If you want to use King Composer, We recommend disabling Elementor, Wilcity Elementor Addon, WPBakery Page Builder, Wilcity WPBakery Addon and else. To disable a plugin, please click on Plugins menu from the sidebar.</span>
                </p>
            </div>
            ...
            <div class="ui bottom attached active tab segment" data-tab="setup">
                <div class="ui icon message">
                    ...
                    <div class="content">
                        ...
                        <div class="ui segment">
                            <p>Hover on Appearance -> Install Plugins -> Make sure that all required plugins are
                                activated. If you do not see Install Plugins under the Appearance, it means you did
                                it.</p>
                        </div>
                        <div class="ui segment">
                            <p>It may takes up to almost 5 minutes, so please patient. But if it takes more than 5
                                minutes, please access to your WordPress folder - Using FileZilla or the same tools -
                                Opening <strong>wp-config.php</strong> -> Put <strong>define("FS_METHOD",
                                    "direct")</strong> before <strong>That's all, stop editing! Happy blogging.</strong>
                                text -> Back to the page and click <strong>Install demo & Setup required
                                    plugins</strong> again.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="ui segment">
                    ...
                    <p class="message ui warning">We recommend using this feature if you migrated from another theme to
                        Wilcity. It will import the home pages only.</p>
                    <form id="wilcity-import-homes" class="wiloke-setup form ui"
                ...
                <div class="ui segment">
                    ...
                    <p class="message ui warning">This import will download everything from Wilcity Server to your
                        site.</p>
                    <p class="message ui warning">If you see a message like this <span>0-media_Part_009_of_18.xml has been imported</span>
                        then button keeps Loading Status more 5 minutes, please refresh your site then click Import
                        button again to continue importing the demo.</p>
                    <?php
                    ...
                    if ($postMaxFilesize < 64) {
                        $configFalse = true;
                        ?>
                        <p class="message ui warning">Your post_max_size is <?php echo $postMaxFilesize ?>. We recommend
                            increasing it to 60 M</p>
                        <?php
                    }
                    ...
                    if ($postMaxFilezie < 64) {
                        $configFalse = true;
                        ?>
                        <p class="message ui warning">Your post_max_size is <?php echo $memoryLimit ?>. We recommend
                            increasing it to 128M</p>
                        <?php
                    }
                    ...
                    if ($maxUploadFilesize < 64) {
                        $configFalse = true;
                        ?>
                        <p class="message ui warning">Your upload_max_filesize is <?php echo $maxUploadFilesize ?>. We
                            recommend increasing it to 60 M</p>
                        <?php
                    }
                    ...
                    if ($maxExecutionTime < 64) {
                        $configFalse = true;
                        ?>
                        <p class="message ui warning">Your max_execution_time is <?php echo $maxExecutionTime ?>. We
                            recommend increasing it to 120</p>
                        <?php
                    }
                    ...
                    if ($memoryLimit < 128) {
                        $configFalse = true;
                        ?>
                        <p class="message ui warning">Your memory_limit is <?php echo $memoryLimit ?>. We recommend
                            increasing it to 128M</p>
                        <?php
                    }
                    ...
                    if ($configFalse) {
                        ?>
                        <p class="message ui warning">To change server configuration, please read <a
                                    href="https://documentation.wilcity.com/knowledgebase/wordpress-and-wilcity-server-requirements/"
                                    target="_blank">WordPress and Wilcity Server Requirements</a></p>
                        <?php
                    }
                    
                    ?>
                    <form id="wilcity-import-everything" class="wiloke-setup form ui"
                          action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" method="POST">
    ...
    public function enqueueScripts($hook)
    {
        ...
        if ((strpos($hook, $this->slug) !== false)) {
            ...
            wp_enqueue_script('wilcity-import', WILCITY_IMPORT_URL.'source/js/script.js', ['jquery'], null, true);
