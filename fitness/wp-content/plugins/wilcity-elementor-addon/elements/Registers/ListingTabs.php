	...
	protected function register_controls()
	{
		...
		$oRepeater->add_control(
			...
			[
				...
				'type'    => Controls_Manager::SELECT,
				'default' => 'listing',
				'options' => SCHelpers::getListingPostTypeOptions()
			],
		);
		$oRepeater->add_control(
			'listing_locations',
        ...
        <div class="<?php echo esc_attr($wrapperClass) ?>">
            <ul class="wilTab_nav__1_kwb wil-tab__nav">
				...
				foreach ($aSettings['listing_tabs'] as $order => $aTab) {
					?>
                    <li><a href="#<?php echo esc_attr($aTab['tab_id']); ?>" class="<?php if ($order == 0) {
							echo 'active';
						} ?>"><?php if (!empty($aTab['icon'])): ?><i
                                class='<?php echo esc_attr($aTab['icon']); ?>'></i><?php endif; ?> <?php echo esc_html($aTab['tab_name']); ?>
                        </a></li>
					<?php
				}
				?>
            ...
            <div class="wilTab_content__2j_o5 wil-tab__content">
				...
				if ($aArgs['toggle_viewmore'] == 'enable') : ?>
                    <div class="btn-view-all-wrap clearfix">
                        <a class="btn-view-all wil-float-right"
                           href="<?php echo SCHelpers::getViewAllUrl($aArgs); ?>"><?php echo esc_html($aArgs['viewmore_btn_name']); ?></a>
                    </div>
				<?php endif; ?>
				<?php
				foreach ($aSettings['listing_tabs'] as $order => $aGridSettings) :
					?>
                    <div id="<?php echo esc_attr($aGridSettings['tab_id']); ?>"
                         class="wilTab_panel__wznsS wil-tab__panel <?php if ($order == 0) {
						     echo 'active';
					     } ?>">
						<?php
