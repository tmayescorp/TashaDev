...
function wilcityMyBankAccounts()
{
    ...
    for ($i = 1; $i <= 4; $i++) {
        if (!empty($aConfiguration['bank_transfer_account_name_'.$i]) &&
            !empty($aConfiguration['bank_transfer_account_number_'.$i]) &&
            !empty($aConfiguration['bank_transfer_name_'.$i])) {
            foreach (
              [
            ...
            <?php foreach ($aBankAccounts as $order => $aAccountInfo): ?>
                <tr>
                    <?php foreach ($aAccountInfo as $key => $val) :
                        $val = empty($val) ? 'X' : $val;
                        ?>
                        <td data-th="<?php echo isset($aFields[$key]) ? esc_html($aFields[$key]) :
                          ''; ?>"><?php echo esc_html($val); ?></td>
                    <?php endforeach; ?>
                </tr>
            <?php endforeach; ?>
            </tbody>
