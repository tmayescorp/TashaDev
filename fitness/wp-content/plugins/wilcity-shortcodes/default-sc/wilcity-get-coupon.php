...
function wilcityGetCoupon($aAtts)
{
    ...
    $icon       = apply_filters('wilcity/filter/coupon/highlight/icon', 'la la-angle-double-right');
    ?>
    <div class="sale-off">
        <div class="row">
            <?php if (!empty($aAtts['highlight'])) : ?>
                <div class="col-sm-4">
                    <a @click.prevent="getCoupon('<?php echo esc_attr($aAtts['code']); ?>', '<?php echo $redirectTo; ?>')"
                       class="<?php echo esc_attr(apply_filters('wilcity/filter/class-prefix',
                         'wilcity-get-counpon')); ?>" href="<?php echo $redirectTo; ?>">
                        <h4 class="color-secondary mt-15 mb-0 text-center"><?php echo esc_attr($aAtts['highlight']); ?>
                            <i class="<?php echo esc_attr($icon); ?>"></i></h4>
                    </a>
                </div>
            <?php endif; ?>
            <div class="<?php echo esc_attr($mainClass); ?>">
                <a @click.prevent="getCoupon('<?php echo esc_attr($aAtts['code']); ?>', '<?php echo $redirectTo; ?>')"
                   class="<?php echo esc_attr(apply_filters('wilcity/filter/class-prefix', 'wilcity-get-counpon')); ?>"
                   href="<?php echo $redirectTo; ?>">
                    ...
                    <?php if (!empty($aAtts['expiry_date'])) : ?>
                        <div><?php echo esc_html__('Expiry date: ', 'wilcity-shortcodes').' '.
                                        esc_html($aAtts['expiry_date']); ?></div>
                    <?php endif; ?>
                </a>
            </div>
        </div>
