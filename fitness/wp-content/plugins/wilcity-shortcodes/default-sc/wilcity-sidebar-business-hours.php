...
function wicitySidebarBusinessHours($aArgs)
{
	...
	if ($hourMode == 'no_hours_available') {
		return '';
	} else if ($hourMode == 'always_open' || $hourMode == 'business_closures') {
		ob_start();
		?>
        <div class="<?php echo esc_attr(apply_filters('wilcity/filter/class-prefix',
            ...
            <div class="content-box_body__3tSRB">
				<?php if ($hourMode == 'always_open') : ?>
                    <h3 class="alway-open wil-btn wil-btn--block wil-btn--secondary wil-btn--round"><?php esc_html_e('Open 24/7',
							'wilcity-shortcodes'); ?></h3>
				<?php else : ?>
                    <h3 class="business-closures wil-btn wil-btn--block wil-btn--round"
                        style="font-weight: bold; color: red"><?php
                        esc_html_e('Business Closures', 'wilcity-shortcodes'); ?></h3>
				<?php endif; ?>
            </div>
        </div>
		<?php
    ...
    <div class="listing-hours_module__38Iy5">
            ...
            <header class="listing-hours_header__1uH8N clearfix active close" data-toggle-button="hour"
                ...
                <div class="wil-float-left">
                    <h4 class="listing-hours_title__sRfK7">
                        ...
                        <span class="listing-hours_status__dQ6Z9 close"><?php echo esc_html__('Closed',
		...
		<?php else:
			...
			?>
            <header class="listing-hours_header__1uH8N clearfix active <?php echo esc_attr($aBusinessStatus['status']); ?>"
				...
				<?php if (isset($aBusinessStatus['firstOpenHour'])) : ?>
                    <div class="wil-float-right">
                    <span class="listing-hours_todayhour__1iK4_">
                        <span class="listing-hours_todayhouritem__2WBlQ"><?php echo WilcityTimeHelper::renderTime($aBusinessStatus['firstOpenHour'],
		                        $timeFormat); ?> - <?php echo WilcityTimeHelper::renderTime($aBusinessStatus['firstCloseHour'],
		                        $timeFormat); ?></span>
                        <?php if (BusinessHours::isSecondHourExists($aBusinessStatus)) : ?>
                            <span class="listing-hours_todayhouritem__2WBlQ"><?php echo WilcityTimeHelper::renderTime($aBusinessStatus['secondOpenHour'],
			                        $timeFormat); ?> - <?php echo WilcityTimeHelper::renderTime($aBusinessStatus['secondCloseHour'],
			                        $timeFormat); ?></span>
                        <?php endif; ?>
                    </span>
            ...
            <ul class="listing-hours_listhours__270Nz list-none">
				...
				foreach ($aBusinessHours as $aDayInfo) :
					if ($aDayInfo['isOpen'] == 'no'):
						$wrapperClass = !$isInvalidFirstHour && $aDayInfo['dayOfWeek'] == $aBusinessStatus['dayKey'] ?
                        ...
                        <li class="<?php echo esc_attr($wrapperClass); ?>">
                            ...
                            <div class="listing-hours_hour__1OuD9">
                                <span class="listing-hours_houritem__3cCSp"><?php esc_html_e('Day Off',
		                                'wilcity-shortcodes'); ?></span>
                            </div>
                        </li>
					<?php
					else:
						if (!$isInvalidFirstHour && $aDayInfo['dayOfWeek'] == $aBusinessStatus['dayKey']) {
                        ...
                        <li class="<?php echo esc_attr($wrapperClass); ?>">
                            <span class="listing-hours_day__2Opo_">
                                <?php echo esc_html($aDefineDaysOfWeek[$aDayInfo['dayOfWeek']]); ?>
                            </span>

                            <div class="listing-hours_hour__1OuD9">
								<?php if ($aDayInfo['firstOpenHour'] == $aDayInfo['firstCloseHour'] &&
									$aDayInfo['firstOpenHour'] === '24:00:00') : ?>
                                    <span class="listing-hours_houritem__3cCSp">
                                        <?php esc_html_e('24 hours', 'wilcity-shortcodes'); ?></span>
								<?php else : ?>
                                    <span class="listing-hours_houritem__3cCSp">
                                    <?php
									...
									<?php if (BusinessHours::isSecondHourExists($aDayInfo)) : ?>
                                        ...
                                        if (!empty($aDayInfo['secondOpenHour'])) {
	                                        echo WilcityTimeHelper::renderTime($aDayInfo['secondOpenHour'],
		                                        $timeFormat);
                                        }

                                        if (!empty($aDayInfo['secondCloseHour'])) {
	                                        echo ' - ';
	                                        echo WilcityTimeHelper::renderTime($aDayInfo['secondCloseHour'],
		                                        $timeFormat);
                                        }
                                        ?>
                                    </span>
