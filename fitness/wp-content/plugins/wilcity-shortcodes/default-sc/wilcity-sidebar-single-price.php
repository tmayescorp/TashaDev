...
function wilcitySidebarSinglePrice($aArgs)
{
    ...
    if (isset($aAtts['isMobile'])) {
        return apply_filters('wilcity/mobile/sidebar/single_price', json_encode([
    ...
    <div class="<?php echo esc_attr(apply_filters('wilcity/filter/class-prefix',
      'wilcity-sidebar-item-single-price content-box_module__333d9')); ?>">
                    ...
                    <div class="price-range_range__19Fut color-primary">
                        <div class="price-range_from__3iV-6">
                            <?php if (!class_exists('NumberFormatter')) : ?>
                                <sup class="price-range_supSig__1pMDY <?php echo esc_attr($symbol); ?>">
                                    <?php echo esc_attr($symbol); ?></sup>
                                <?php echo esc_html($price); ?>
                            <?php else : ?>
                                <?php echo Price::renderPrice($price); ?>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
