...
function wilcityRenderSidebarCoupon($aArgs, $postID = '')
{
	...
	if (isset($aCoupon['popup_image']) && !empty($aCoupon['popup_image'])) {
		...
		if (isset($wiloke->aThemeOptions['listing_coupon_popup_img']) &&
			!empty($wiloke->aThemeOptions['listing_coupon_popup_img'])) {
			$img = isset($wiloke->aThemeOptions['listing_coupon_popup_img']['url']) &&
			!empty($wiloke->aThemeOptions['listing_coupon_popup_img']['url']) ?
				esc_url($wiloke->aThemeOptions['listing_coupon_popup_img']['url']) :
				wp_get_attachment_image_url($wiloke->aThemeOptions['listing_coupon_popup_img']['id'],
					'large');
		}
	}
	...
	if (isset($aCoupon['expiry_date']) && is_numeric($aCoupon['expiry_date'])) {
		...
		$aCoupon['expiry_date']
			= date(get_option('date_format') . ' ' . get_option('time_format'), $aCoupon['expiry_date']);
	}

	$fbShareUrl = add_query_arg(
		[
			"content" => json_encode($aCoupon),
			"quote"   => $fbQuote
    ...
    <div class="<?php echo esc_attr($wrapperClass); ?>">
        ...
        <div class="content-box_body__3tSRB">
            <div class="content-box_body__3tSRB">
				...
				<?php endif; ?>
				<?php if (!empty($aCoupon['code'])): ?>
                    <div class="<?php echo esc_attr(apply_filters('wilcity/filter/class-prefix',
						'wilcity-coupon-code color-secondary')); ?>">
                        <span><?php echo esc_html($aCoupon['code']); ?></span>
                    </div>
				<?php endif; ?>
                ...
                <a class="<?php echo esc_attr(apply_filters('wilcity/filter/class-prefix',
                   ...
                   target="_blank"
                   href="https://www.facebook.com/sharer/sharer.php?u=<?php echo urlencode($fbShareUrl); ?>">
                ...
                <a target="_blank" class="wil-btn mt-10 wil-btn--twitter wil-btn--md wil-btn--block wil-btn--round"
                   href="//twitter.com/share?text=<?php echo urlencode($aCoupon['title']); ?>&url=<?php echo urlencode(get_permalink($post->ID)); ?>&hashtags=<?php echo esc_attr($twitterHash); ?>">
				...
				<?php if (!empty($aCoupon['redirect_to'])) : ?>
                    <a href="<?php echo esc_url($aCoupon['redirect_to']) ?>" class="wil-btn mt-10 wil-btn--primary
                    wil-btn--md wil-btn--block wil-btn--round wil-coupon-redirect-to" target="_blank" rel="nofollow">
                        <i class="la la-link"></i>
						<?php esc_html_e('Open website', 'wilcity-shortcodes'); ?>
                    </a>
				<?php endif; ?>
