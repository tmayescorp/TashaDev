...
function wilcity_feature_slider_item($post, $imgSize='wilcity_380x215', $atts=array()){
    ...
    $featuredImgUrl = GetSettings::getFeaturedImg($post->ID, $imgSize);
    $headerClass = 'listing_header__2pt4D';
    if ( SingleListing::isClaimedListing($post->ID, true) ){
    ...
    <article class="<?php echo esc_attr(apply_filters('wilcity/filter/class-prefix', 'wilcity-listing-slider')); ?> loading listing_module__2EnGq wil-shadow listing_style2__2PwZl listing_dark__1rJrd  js-listing-module">
        <div class="listing_firstWrap__36UOZ">
            <header class="<?php echo esc_attr($headerClass); ?>">
	            <?php do_action('wilcity/wilcity-shortcode/core/wilcity_listing_slider_item/before-header', $post); ?>
                <?php SCHelpers::renderAds($post, 'LISTINGS_SLIDER'); ?>
                <a href="<?php echo esc_url(get_permalink($post->ID)); ?>">
	                <?php do_action('wilcity/wilcity-shortcode/core/wilcity_listing_slider_item/after-open-permalink',
                    $post); ?>
	                ...
	                <?php endif; ?>
	                <?php do_action('wilcity/wilcity-shortcode/core/wilcity_listing_slider_item/before-close-permalink', $post); ?>
                </a>
	            <?php SCHelpers::renderClaimedBadge($post->ID); ?>
                <?php do_action('wilcity/wilcity-shortcode/core/wilcity_listing_slider_item/after-header', $post); ?>
            </header>
            <div class="listing_body__31ndf">
	            ...
	            <?php endif; ?>
				<?php SCHelpers::renderTitle($post, 'h3'); ?>
