...
class WooCommerceController
{
	...
	/**
	 * @var $oWooCommerce \Automattic\WooCommerce\Client
	 */
	protected $oWooCommerce;
	protected $oUserMeta;
	protected $aRequiredCheckoutFields;
	protected $aAllowedCountries;
	protected $aCountryStates;
	protected $productID;
	public    $errAuthMsg      = '';
	protected $aCacheCartItems;
	protected $cartItems       = 'wilcity_my_cart_items';
	protected $cartTotalKey    = 'wilcity_cart_total';
	protected $cartSubTotalKey = 'wilcity_cart_sub_total';
	protected $cartQuantity    = 'wilcity_cart_quantity';

	/**
	 * @var $oWooCommerce \WC_Product
	 */
	protected $oProduct;
	/**
	 * Details array
	...
	protected function responseAuthExpired()
	{
		return [
			'status' => 'error',
			'msg'    => 'It seems you still not setup WooCommerce Authentication yet or it was expired. Please go to documentation.wilcity.com -> Search for WooCommerce on Wilcity App and follow this tutorial'
		];
	}
	...
	protected function getCartItems($isFocus = false)
	{
		...
		if (!$isFocus) {
			if (isset($this->aCacheCartItems[$this->userID])) {
				return $this->aCacheCartItems[$this->userID];
			}
		}
		...
		try {
			...
			$aCartItems
				= isset($aSessions['cart']) ? maybe_unserialize($aSessions['cart']) : [];
			$this->aCacheCartItems[$this->userID] = $aCartItems;
	...
	protected function isProductInCart($productID, $isFocus = false)
	{
		$aCartItems = $this->getCartItems($isFocus);
	...
	protected function getCartKey($productID, $isFocus = false)
	{
		$aCartItems = $this->getCartItems($isFocus);
	...
	public function productSkeleton($product, $post, $aExcludes = [])
	{
		...
		$aResponse = [
			...
			'isAddedToCart'     => $this->isProductInCart($productID),
			'cartKey'           => $this->getCartKey($productID),
			'aListingIDs'       => WooCommerce::getListingIDsByProductID($productID)
		];

		if ($product->get_stock_quantity() !== null) {
			$aResponse['stockQuantity'] = $product->get_stock_quantity();
		}
	...
	public function productCartItemSkeleton($product, $aCartData)
	{
		...
		$featuredImgID = isset($aCartItem['variation_id']) && has_post_thumbnail($aCartItem['variation_id']) ?
			$aCartItem['variation_id'] :
			$productID;
		...
		return [
			'id'            => abs($productID),
			'variationID'   => isset($aCartItem['data']) && !empty($aCartItem['data']) ?
				$aCartItem['data']->get_variation_id() : 0,
			...
			'salePriceHTML' => wc_price($product->get_sale_price()),
			'oFeaturedImg'  => $aFeaturedImg,
			'aVariations'   => $isVariation ? $this->getCartItemData($aCartItem) : [],
			'attributes'    => $aCartItem['variation'] ?? null,
			'listingIDs'    => []
		];
	}
	...
	protected function getMyCartSkeleton($userID)
	{
		...
		$aCartItems = get_user_meta($userID, '_woocommerce_persistent_cart_' . get_current_blog_id(), true);

		if (empty($aCartItems) || empty($aCartItems['cart'])) {
			return [];
		}

		$aCartItems = $aCartItems['cart'];


		$aProducts = [];
		$totalPrice = floatval(get_user_meta($userID, $this->cartTotalKey, true));
		$totalItems = floatval(get_user_meta($userID, $this->cartQuantity, true));
		
		foreach ($aCartItems as $aCartItem) {
			if (!empty($aCartItem)) {
				$aProducts['items'][] = $this->productCartItemSkeleton(wc_get_product($aCartItem['product_id']),
					$aCartItem);
//				$totalPrice = $totalPrice + floatval($aCartItem['line_total']);
//				$totalItems = $totalItems + $aCartItem['quantity'];
			}
		}

		$aCouponCode = WC()->cart->get_applied_coupons();
		$aCouponDiscount = WC()->cart->get_coupon_discount_totals();
		if (!empty($aCouponCode && is_array($aCouponCode))) {
			$couponDiscountTotal = 0;
			foreach ($aCouponDiscount as $discount) {
				$couponDiscountTotal += $discount;
			}
			$aProducts['applied_coupon'] = end($aCouponCode);
			$aProducts['totalPrice'] = $totalPrice;
			$aProducts['totalPriceHTML'] = wc_price($totalPrice);
			$aProducts['regularPrice'] = $couponDiscountTotal + $totalPrice;
			$aProducts['regularPriceHTML'] = wc_price($aProducts['regularPrice']);
		} else {
			$aProducts['totalPrice'] = $totalPrice;
			$aProducts['totalPriceHTML'] = wc_price($totalPrice);
		}
	...
	{
		if (isset($this->aCountryStates[$country])) {
			return $this->aCountryStates[$country];
		}

		$oCountries = new \WC_Countries();
		$aRawStates = $oCountries->get_states($country);
		if (empty($aRawStates)) {
			return [];
		}

		foreach ($aRawStates as $state => $name) {
			$aStates[] = [
				'id'       => $state,
				'name'     => $name,
				'selected' => $this->getBillingField('billing_state') == $state
			];
		}

		$this->aCountryStates[$country] = $aStates;
	...
	{
//        $allowCountriesMode = get_option('woocommerce_allowed_countries');
		$country = $this->getBillingField($prefix . 'country');
		$aFields = [
			[
				'type'           => 'text',
			...
			[
				...
				'options'        => empty($country) ? [] : $this->getCountryStates($country)
			],
			[
				'type'           => 'text',
	...
	public function getStateOptions(\WP_REST_Request $oRequest)
	{
		if (empty($country = $oRequest->get_param('country'))) {
			return [
				'status' => 'error',
				'msg'    => 'The Country code is required'
			];
		}

		return [
			'status'  => 'success',
			'options' => $this->getCountryStates($country)
		];
	}
	...
	protected function auth()
	{
		try {
			...
			return !empty($this->oWooCommerce);
		}
		catch (\Exception $oException) {
