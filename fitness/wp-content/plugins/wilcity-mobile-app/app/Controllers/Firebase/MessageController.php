...
//        add_action('wp_ajax_nopriv_build_chat_room', [$this, 'ajaxBuildChatRoom']);
	...
	private function buildUserId($user1, $user2): string
	{
		...
		try {
			if (empty($this->db)) {
				...
				if ($this->db->getValue() === null) {
					$this->db = FirebaseDB::getDB()->getReference(
						'messages/chats/' . $this->buildUserId($user2, $user1)
					);
				}
			}
			setcookie("wilcity_firebase_issue", "", time() - 1);
			return $this->db;
		}
		catch (Exception|DatabaseException $oException) {
			setcookie("wilcity_firebase_issue", $oException->getMessage());
		}
	}
	...
	private function buildChatRoom($receiverID, $senderID): bool
	{
		$this->referMsgDB($receiverID, $senderID);
		if (!$this->db->getChild('fUser')->getSnapshot()->exists()) {
