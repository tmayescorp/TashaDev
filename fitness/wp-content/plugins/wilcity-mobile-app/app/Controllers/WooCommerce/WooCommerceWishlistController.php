...
class WooCommerceWishlistController extends WooCommerceController
{
	...
	public function __construct()
	{
		add_action('rest_api_init', function () {
			...
			register_rest_route(WILOKE_PREFIX . '/' . WILOKE_MOBILE_REST_VERSION, '/wc/wishlists', [
				[
					'methods'             => \WP_REST_Server::READABLE,
					'callback'            => [$this, 'getWishlists'],
					'permission_callback' => '__return_true'
	...
	private function addYITHWishList($productID, $userID, $quantity = 1)
	{
		...
		if ($this->isYITHProductInWishlist($productID, $wishlistID, $userID)) {
			return [
				'status' => 'error',
				'msg'    => wilcityAppGetLanguageFiles('productHasBeenAddedToWishlistAlready')
			];
		}
	...
	public function getWishlists(\WP_REST_Request $oRequest)
	{
		...
		$aStatus = $this->isInstalledWITHWCWL();
	...
	private function countYITHWishListProduct($wishlist_token)
	{
		...
		if (false === $count) {
			...
			$transient_name = !empty($wishlist_token) ? ('yith_wcwl_wishlist_count_' . $wishlist_token) :
				('yith_wcwl_user_default_count_' . get_current_user_id());
	...
	public function deleteWishlist(\WP_REST_Request $oRequest)
	{
		...
		if ($count != 0) {
			if ($this->deleteYITHWishListItem($productID, $wishListID, $userID)) {
				return [
					'status' => 'success',
					'msg'    => sprintf(wilcityAppGetLanguageFiles('removedProductFromWishlist'),
	...
	private function countYITHTotalProductsInUserWishList($userID, $wishlist_token = '')
	{
		...
		if (false === $count) {
			...
			$transient_name = !empty($wishlist_token) ? ('yith_wcwl_wishlist_count_' . $wishlist_token) :
				('yith_wcwl_user_default_count_' . $userID);
			set_transient($transient_name, $count, WEEK_IN_SECONDS);
		}
	...
	private function getYITHWishListItems($userID, $aArgs = [])
	{
		...
		$aProducts = [];
		foreach ($aWishListItems as $oItem) {
			$aProduct = $this->productSkeleton(wc_get_product($oItem->get_product_id()), get_post
			($oItem->get_product_id()));
			$aProduct['wishListID'] = abs($oItem->get_wishlist_id());
			$aProduct['dateadded'] = $oItem->get_date_added();
			$aProduct['wishlistSlug'] = $oItem->get_wishlist_slug();
			$aProduct['wishlistName'] = $oItem->get_wishlist_name();
			$aProduct['wishlistToken'] = $oItem->get_wishlist_token();
			$aProducts[] = $aProduct;
		}
