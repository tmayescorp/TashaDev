...
trait JsonSkeleton
{
	...
	protected $aAddListingSkeletonCache = [];

	protected function getAddListingSkeleton($postType)
	{
		if (isset($this->aAddListingSkeletonCache[$postType])) {
			return $this->aAddListingSkeletonCache[$postType];
		}

		$oInit = new AddListingFieldSkeleton($postType);
		$this->aAddListingSkeletonCache[$postType] = $oInit;
	...
	protected function buildCachingFile($fileName): string
	{
	...
	public function getOptionField($key = '', $getWPMLHomePage = false)
	{
		...
		if (isset($this->aThemeOptions[$key])) {
			if (WPML::isActive() && $getWPMLHomePage) {
				return WPML::getPageIdOfCurrentLanguage($this->aThemeOptions[$key]);
			} else {
				return $this->aThemeOptions[$key];
			}
		}
	...
	{
		$aGallery = GetSettings::getPostMeta($post->ID, 'gallery', '', '', true);
		$aReview['ID'] = absint($post->ID);
		$aReview['userID'] = absint($post->post_author);
		$aReview['postStatus'] = $post->post_status;
		$aReview['postTitle'] = AppHelpers::replaceHTMLEntities(get_the_title($post->ID));
		$aReview['permalink'] = get_permalink($post->ID);
		$aReview['shareURL'] = GetSettings::getShareReviewURL(get_permalink($postParent), $post->ID);
		$aReview['postContent'] = do_shortcode(get_post_field('post_content', $post->ID));
		$aReview['postExcerpt'] = get_the_excerpt($post->ID);
		$aReview['postDate'] = Time::getPostDate($post->post_date);
		$aReview['countLiked'] = absint(ReviewMetaModel::countLiked($post->ID));
		$aReview['countShared'] = absint(SharesStatisticController::renderShared($post->ID, false));
		$aReview['countDiscussions'] = absint(ReviewMetaModel::countDiscussion($post->ID));
	...
	public function getPostMeta($aData, $isTestMode = false)
	{
		...
		switch ($aData['metaKey']) {
			...
			case 'listing_content':
				if ($isTestMode) {
					return nl2br(get_post_field('post_content', $postID));
				}
				$aContent = nl2br(get_post_field('post_content', $postID));
			...
			case 'events':
				...
				$oEventQuery = new \WP_Query(
					WPML::addFilterLanguagePostArgs(SearchFormController::buildQueryArgs($aArgs))
				);
			...
			case 'reviews':
				...
				$aContent['mode'] = GetSettings::getOptions(General::getReviewKey('mode', get_post_type($postID)),
					false, true);
				$aContent['mode'] = empty($aContent['mode']) ? 5 : absint($aContent['mode']);
				...
				if ($query->have_posts()) {
					$aContent['total'] = absint($query->found_posts);
					$aContent['maxPages'] = absint($query->max_num_pages);
	...
	protected function getUserProfile($userID, $excludeFollowContact = false)
	{
		...
		$allow = GetSettings::getUserMeta($userID, 'send_email_if_reply_message');
		$aUserInfo = [
			'oBasicInfo' => apply_filters('wilcity/filter/wilcity-mobile-app/app/Controllers/JsonSkeleton/getUserProfile/oBasicInfo',
				[
					'userID'                      => $userID,
					...
					'send_email_if_reply_message' => $allow === 'yes'
				], $userID)
		];
	...
	protected function eventCommentItem($post)
	{
		return [
			...
			'countDiscussions' => absint(GetSettings::countNumberOfChildrenReviews($post->ID)),
			'countLiked'       => absint(ReviewMetaModel::countLiked($post->ID)),
			'countShared'      => absint(SharesStatisticController::renderShared($post->ID, false)),
	...
	protected function detectShortcodeType($content): string
	{
	...
	{
		$aRenderMachine = wilokeListingToolsRepository()->get(
			'listing-settings:sidebar_settings', true
		)->sub('renderMachine');
		//        $aSetting['post_id'] = $this->listingID;
		...
		if (!isset($aRenderMachine[$aSetting['key']])) {
			...
			$val
				= do_shortcode("[" . $aRenderMachine[$aSetting['key']] . " atts='" .
				stripslashes(json_encode($aSetting)) . "'/]");
			if (!empty($val)) {
	...
	public function getNavigationAndHome($post)
	{
		...
		foreach ($aSettings as $key => $aSection) {
			...
			if (has_filter(
				...
				$aSettings[$key] = $aSection;
			}

			$aSection['category'] = $aSection['baseKey'];
			...
			if ($aSection['isShowOnHome'] == 'yes') {
				...
				if (!empty($val)) {
					if (isset($aSection['baseKey'])) {
						$aSection['category'] = isset($aSection['content']) ?
							$this->detectShortcodeType($aSection['content']) : '';
					}
		...
		$aNavigation = array_filter($aSettings, function ($aSection) {
			if (in_array($aSection['baseKey'], ['custom_section', 'post', 'article', 'my_advanced_products'])) {
				return false;
			}
	...
	{
		return ReviewMetaModel::getGeneralReviewData(get_post($postID));
	}
	...
	public function listingSkeleton($post, $aExcludes = [], $aAtts = []): array
	{
		...
		$toggleReview = GetSettings::getOptions(General::getReviewKey('toggle', $post->post_type), false, true);
		...
		$aResponse = [
			...
			'timezone'        => BusinessHours::getTimezone($post->ID),
			...
			'businessHours'   => GetSettings::getBusinessHours($post->ID),
			'hourMode'        => $hourMode,
			...
			'oReview'         => [
				'mode'    => absint(GetSettings::getOptions(General::getReviewKey('mode', $post->post_type), false,
					true)),
		...
		if ($post->post_type == 'event') {
			$aResponse['oAuthor']['displayName'] = GetSettings::getEventHostedByName($post);
		}
		...
		if (isset($aAtts['TYPE'])) {
			if ($aAtts['TYPE'] == 'LISTINGS') {
				...
				$aResponse['isAds'] = SCHelpers::renderAds($post, 'GRID', true);
				if ($aResponse['isAds'] == 'no') {
					$aResponse['isAds'] = SCHelpers::renderAds($post, 'LISTINGS_SLIDER', true);
				}
			}
		}
