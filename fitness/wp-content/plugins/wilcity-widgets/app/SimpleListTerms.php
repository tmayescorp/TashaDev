...
class SimpleListTerms extends \WP_Widget
{
    ...
    public function form($aInstance)
    {
        ...
        $aOrderBy  = ['count' => 'Count', 'id' => 'Id', 'slug' => 'Slug', 'name' => 'Name', 'none' => 'None']
        ?>
        ...
        <div class="widget-group">
            ...
            <input type="text" class="widefat" name="<?php echo $this->get_field_name('number_of_terms'); ?>"
                   id="<?php echo $this->get_field_id('number_of_terms'); ?>"
                   value="<?php echo esc_attr($aInstance['number_of_terms']); ?>">
        </div>
        <div class="widget-group">
        ...
        <div class="widget-group">
            ...
            <select class="widefat" name="<?php echo $this->get_field_name('order'); ?>"
                ...
                <?php foreach (['DESC' => 'DESC', 'ASC' => 'ASC'] as $option => $name): ?>
                    <option value="<?php echo esc_attr($option); ?>" <?php selected($option,
                      $aInstance['orderby']); ?>><?php echo esc_html($name); ?></option>
                <?php endforeach; ?>
            </select>
    ...
    public function widget($aAtts, $aInstance)
    {
        ...
        if (!empty($aInstance['title'])) {
            echo $aAtts['before_title']; ?><i class="la la-th-list"></i>
            <span><?php echo esc_html($aInstance['title']); ?><?php echo $aAtts['after_title'];
        }
        echo '<ul>';
