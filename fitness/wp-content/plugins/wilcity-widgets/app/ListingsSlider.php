...
class ListingsSlider extends \WP_Widget
{
    ...
    public function form($aInstance)
    {
        ...
        $aTaxonomies = [
          ...
          'ignore'           => 'I do not want to use Related By feature'
        ];
        ...
        <div class="widget-group">
            ...
            <select class="widefat" name="<?php echo $this->get_field_name('post_type'); ?>"
                ...
                <?php foreach ($aPostTypes as $option => $name): ?>
                    <option value="<?php echo esc_attr($option); ?>" <?php selected($option,
                      $aInstance['post_type']); ?>><?php echo esc_html($name); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="widget-group">
            ...
            <select class="widefat" name="<?php echo $this->get_field_name('orderby'); ?>"
                ...
                <?php foreach ($aOrderbyOptions as $orderby => $name): ?>
                    <option value="<?php echo esc_attr($orderby); ?>" <?php selected($orderby,
                      $aInstance['orderby']); ?>><?php echo esc_html($name); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="widget-group">
            ...
            <select class="widefat" name="<?php echo $this->get_field_name('order'); ?>"
                ...
                <?php foreach ($aOrderOptions as $order => $name): ?>
                    <option value="<?php echo esc_attr($order); ?>" <?php selected($order,
                      $aInstance['order']); ?>><?php echo esc_html($name); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="widget-group">
            <label for="<?php echo $this->get_field_id('taxonomy'); ?>">Related By</label>
            <p><i>Self Taxonomy option: For instance, if a customer is in Listing Location page, it will get the
                    listings that belongs to Listing Location category. But the he/she visits a Listing Category page,
                    it will get the listings belongs to Listing Category. In other words, it depends on where the
                    taxonomy page your customer in.</i></p>
            <select class="widefat" name="<?php echo $this->get_field_name('taxonomy'); ?>"
                ...
                <?php foreach ($aTaxonomies as $option => $name): ?>
                    <option value="<?php echo esc_attr($option); ?>" <?php selected($option,
                      $aInstance['taxonomy']); ?>><?php echo esc_html($name); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="widget-group">
            ...
            <input type="text" class="widefat" name="<?php echo $this->get_field_name('post_ids'); ?>"
                   id="<?php echo $this->get_field_id('post_ids'); ?>"
                   value="<?php echo esc_attr($aInstance['post_ids']); ?>">
        </div>
        <div class="widget-group">
            <label for="<?php echo $this->get_field_id('number_of_posts'); ?>">Number of posts</label>
            <input type="text" class="widefat" name="<?php echo $this->get_field_name('number_of_posts'); ?>"
                   id="<?php echo $this->get_field_id('number_of_posts'); ?>"
                   value="<?php echo esc_attr($aInstance['number_of_posts']); ?>">
        </div>
        <div class="widget-group">
            <label for="<?php echo $this->get_field_id('autoplay'); ?>">Auto Play?</label>
            <select class="widefat" name="<?php echo $this->get_field_name('autoplay'); ?>"
    ...
    public function widget($aAtts, $aInstance)
    {
        ...
        if (!empty($aInstance['title'])): ?>
            <?php echo $aAtts['before_title']; ?><i class="la la-th-list"></i>
            <span><?php echo esc_html($aInstance['title']); ?></span><?php echo $aAtts['after_title']; ?>
        <?php endif ?>
        <?php
        if (!function_exists('wilcity_render_grid_item')):
            ...
            <div class="content-box_body__3tSRB">
                <div class="swiper__module swiper-container swiper--button-abs"
                     data-options='{"slidesPerView":"auto","spaceBetween":10,"speed":1000,"autoplay":<?php if ($aInstance['autoplay'] ==
                                                                                                               'yes') : ?>true<?php else: ?>false<?php endif; ?>,"loop":true}'>
                    ...
                    <div class="swiper-wrapper">
                        ...
                        while ($query->have_posts()) {
                            ...
                            wilcity_render_grid_item($query->post,
                              ['img_size' => $aAtts['img_size'], 'isSlider' => true, 'style' => 'grid']);
                        }
                        wp_reset_postdata();
                        ?>
                    </div>
