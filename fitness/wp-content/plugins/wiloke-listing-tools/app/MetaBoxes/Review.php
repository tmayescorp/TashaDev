...
class Review {
	...
	public function renderMetaboxFields(){
		...
		if (General::detectCurrentPostType() !== 'review') {
		    return false;
        }
		...
		if ( !empty($parentID) ){
			$aDetails = GetSettings::getOptions(General::getReviewKey('details', get_post_type($parentID)), false, true);
			...
			if ( !empty($aDetails) ){
				...
				$aReviewDetails = array(
					'id'            => 'review_details',
					'title'         => 'Review Categories Score (Score Scale: ' . GetSettings::getOptions(General::getReviewKey('mode', get_post_type($parentID)), false, true) . ')',
