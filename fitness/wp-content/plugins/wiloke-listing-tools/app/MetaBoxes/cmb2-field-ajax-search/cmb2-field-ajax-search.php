    ...
    class CMB2_Field_Ajax_Search
    {
        ...
        public function render($field, $value, $object_id, $object_type, $field_type)
        {
            ...
            echo $field_type->input([
                ...
                'data-query-args'  => $field->args('query_args') ?
                    htmlspecialchars(json_encode($field->args('query_args')), ENT_QUOTES, 'UTF-8') : ''
            ]);
        ...
        {
            $object_type =
                str_replace('cmb2_pre_field_display_', '', str_replace('_ajax_search', '', current_filter()));
            
            ob_start();
            
            $field->peform_param_callback('before_display_wrap');
            
            printf("<div class=\"cmb-column %s\" data-fieldtype=\"%s\">\n", $field->row_classes('display'),
                $field->type());
        ...
        public function setup_admin_scripts()
        {
            ...
            wp_register_script('jquery-autocomplete-ajax-search',
                plugins_url('js/jquery.autocomplete.min.js', __FILE__), ['jquery'], self::VERSION, true);
            wp_register_script('cmb-ajax-search', plugins_url('js/ajax-search.js', __FILE__),
                ['jquery', 'jquery-autocomplete-ajax-search', 'jquery-ui-sortable'], self::VERSION, true);
            
            wp_localize_script('cmb-ajax-search', 'cmb_ajax_search', [
