...
class ClaimListing
{
    public function __construct()
    {
        ...
        add_action('save_post_claim_listing', [$this, 'saveClaimListing'], 10, 2);
    }
        ...
        foreach ($aClaimFields as $aClaimField) {
            switch ($aClaimField['type']) {
                ...
                case 'checkbox':
                    ...
                    $aOptions = [];
                    foreach ($parseOptions as $key => $option) {
                    ...
                    $aFields[] = [
                        'type'        => 'wiloke_field',
                        'fieldType'   => $aClaimField['type'] == 'radio' ? 'select' : 'multicheck',
                        'save_fields' => false,
                        'id'          => 'wilcity_claimer_info:' . $aClaimField['key'],
                        'name'        => $aClaimField['label'],
                        'options'     => $aOptions
                ...
                case 'claimPackage':
                    ...
                    if (!class_exists('WilcityPaidClaim\Register\RegisterClaimSubMenu')) {
                        $msg
                            = 'Wilcity Paid Claim plugin is disabled, You have to activate this plugin to use Paid Claim feature';
                    } else {
                        $aSettings = GetSettings::getOptions(RegisterClaimSubMenu::$optionKey);
                        if (!isset($aSettings['toggle']) || ($aSettings['toggle'] == 'disable')) {
                            $msg
                                = 'Paid Claim feature is disabled, please go to Wiloke Submission -> Claim Settings to enable this feature';
                        }
                    }
