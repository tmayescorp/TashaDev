...
//		add_action('init', array($this, 'updateLatitudeLongitudeForEachListing'));
        add_action('init', [$this, 'addMoreListingPostTypes'], 1);
        add_filter('posts_where', [$this, 'editListTableQuery'], 10, 2);
    ...
    private function findPostTypeSetting(array $aPostTypes, $key)
    {
        foreach ($aPostTypes as $aPostType) {
            if ($aPostType['key'] == $key) {
                return $aPostType;
            }
        }
    }
    ...
    public function register()
    {
                ...
                switch ($aPostType['key']) {
                    case 'event':
                        if ($aPostType['name'] !== 'Event') {
                            ...
                            $aPostTypePatterns['event'] = apply_filters('wilcity/filter/register-post-types/event',
                                $aPostTypePatterns['event'], $aPostType['key']);
                        }
                    ...
                    default:
                        ...
                        foreach ($aPostTypePatterns['listing']['labels'] as $key => $label) {
                            if (in_array($key, ['menu_name', 'name_admin_bar', 'name', 'all_items'])) {
                                $aGeneratePostType['labels'][$key] = $aPostType['name'];
                            } else {
                                $aGeneratePostType['labels'][$key] = $aPostType[$key] ?? $label;
                            }
                        }
                        ...
                        $aGeneratePostType = apply_filters('wilcity/filter/register-post-types/listings',
                            $aGeneratePostType, $aPostType['key']);
                        $postTypeKey = apply_filters('wilcity/filter/register-post-types/post-type-key',
                            $aPostType['key']);
        ...
        foreach ($aTaxonomies as $tax => $aArgs) {
            if (!empty($aThemeOptions) && isset($aThemeOptions[$tax . '_slug']) &&
                !empty($aThemeOptions[$tax . '_slug'])) {
