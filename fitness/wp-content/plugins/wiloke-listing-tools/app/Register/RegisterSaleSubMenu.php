...
class RegisterSaleSubMenu
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_wiloke_delete_sales', [$this, 'deleteItems']);
        add_action('wp_ajax_wiloke_submission_export_sales', [$this, 'exportFile']);
    }
    ...
    public function exportFile()
    {
        ...
        foreach ($this->aSales as $aInfo) {
            ...
            $csv_row .= $aInfo['ID'] . ', ' . $oUser->display_name . ', ' . $oUser->user_email . ', ' .
                get_the_title($aInfo['planID']) . ', ' . $aInfo['status'] . ', ' . $aInfo['gateway'] . ', ' .
                $aInfo['createdAt'] . "\n";
        }
    ...
    {
        add_submenu_page($this->parentSlug, esc_html__('Sales', 'wiloke'), esc_html__('Sales', 'wiloke'),
            'administrator', $this->slug, [$this, 'showSales']);
    }
    ...
    protected function fetchItems($aQuery)
    {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS $tblSession.* FROM $tblSession WHERE billingType='" .
        ...
        if (isset($aQuery['package_id']) && !empty($aQuery['package_id']) && $aQuery['package_id'] !== 'any') {
            $sql .= $concat . " $tblSession.planID=" . absint($aQuery['package_id']);
            $aParams[] = $aQuery['package_id'];
            $concat = " AND";
        }

        $additionalQuery = '';
        if (isset($aQuery['payment_status']) && !empty($aQuery['payment_status']) &&
            ($aQuery['payment_status'] !== 'all')) {
        ...
        if (isset($aQuery['date']) && $aQuery['date'] !== 'any') {
            if ($aQuery['date'] === 'this_week') {
                $additionalQuery .= $concat .
                    " DATE_SUB(CURDATE(), INTERVAL DAYOFWEEK(CURDATE()) DAY) <= $tblSession.createdAt";
            } elseif ($aQuery['date'] === 'this_month') {
                $additionalQuery .= $concat .
                    " $tblSession.createdAt >= DATE_SUB(CURDATE(), INTERVAL DAYOFMONTH(CURDATE()) DAY) ";
            } else {
                ...
                $additionalQuery .= $concat . " ($tblSession.createdAt BETWEEN '" . $wpdb->_real_escape($from) .
                    "' AND '" . $wpdb->_real_escape($to) . "')";
            }
        }
        ...
        $sql .= " ORDER BY $tblSession.ID DESC LIMIT " . $wpdb->_real_escape($this->postPerPages) . " OFFSET " .
            absint($offset);
    ...
    public function showSales()
    {
        ...
        $this->postPerPages = isset($_REQUEST['posts_per_page']) && !empty($_REQUEST['posts_per_page']) ?
            $_REQUEST['posts_per_page'] : $this->postPerPages;
        $this->gateway = isset($_REQUEST['gateway']) && !empty($_REQUEST['gateway']) ? $_REQUEST['gateway'] :
            $this->gateway;
                ...
                <div class="ui segment">
                    <form action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" method="POST"
                        ...
                        <button class="js_export button ui basic purple"><?php esc_html_e('Export Payments',
                                'wiloke'); ?></button>
                        <button class="js_delete_sales button ui basic red"><?php esc_html_e('Delete Checked Items',
                                'wiloke'); ?></button>
                        <a class="button ui basic green" target="_blank"
                           href="<?php echo add_query_arg(['page' => 'wilcity-addnew-payment'], admin_url('admin.php')) ?>">
                            <?php esc_html_e('Add New Payment', 'wiloke'); ?></a>
                    </form>
                </div>
            <?php endif; ?>
