...
class RegisterInvoiceSubMenu
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_wiloke_submission_export_invoices', [$this, 'exportFile']);
        add_action('wp_ajax_wiloke_delete_invoices', [$this, 'deleteInvoices']);
    }
    ...
    public function enqueueScripts($hook)
    {
        if (strpos($hook, $this->slug) !== false) {
            ...
            wp_enqueue_style('wiloke-submission-table',
                WILOKE_LISTING_TOOL_URL . 'admin/source/css/wiloke-submission-table.css', false,
    ...
    protected function fetchInvoices($aQuery)
    {
        ...
        $this->postPerPages
            = isset($aQuery['posts_per_page']) && !empty($aQuery['posts_per_page']) ? $aQuery['posts_per_page'] :
            $this->postPerPages;
        $offset = ($this->paged - 1) * $this->postPerPages;

        $sql
            = "SELECT SQL_CALC_FOUND_ROWS $invoicesTbl.*, $sessionTbl.ID as paymentID, $sessionTbl.userID, $sessionTbl.gateway, $sessionTbl.billingType, $sessionTbl.planID FROM $invoicesTbl LEFT JOIN $sessionTbl ON ($invoicesTbl.paymentID=$sessionTbl.ID)";
        $concat = " WHERE";

        if (isset($aQuery['planID']) && $aQuery['planID'] !== 'any') {
            $sql .= $concat . " $sessionTbl.planID=" . absint($aQuery['planID']);
        ...
        if (isset($aQuery['date']) && $aQuery['date'] !== 'any') {
            if ($aQuery['date'] === 'this_week') {
                $additionalQuery .= $concat .
                    " DATE_SUB(CURDATE(), INTERVAL DAYOFWEEK(CURDATE()) DAY) <= $sessionTbl.createdAt";
            } elseif ($aQuery['date'] === 'this_month') {
                $additionalQuery .= $concat .
                    " $sessionTbl.created_at >= DATE_SUB(CURDATE(), INTERVAL DAYOFMONTH(CURDATE()) DAY) ";
            } else {
                ...
                $additionalQuery .= $concat . " ($sessionTbl.createdAt BETWEEN '" . $wpdb->_real_escape($from) .
                    "' AND '" .
                    $wpdb->_real_escape($to) . "')";
            }
        }
        ...
        $sql .= " ORDER BY $invoicesTbl.ID DESC LIMIT " . absint($this->postPerPages) . " OFFSET " . absint($offset);
    ...
    public function exportFile()
    {
        ...
        foreach ($this->aInvoices as $aInfo) {
            ...
            $csv_row .= $aInfo['ID'] . ', ' . $oUser->display_name . ',' . $oUser->user_email . ', ' .
                $aInfo['paymentID'] . ', ' .
                get_the_title($aInfo['planID']) . ', ' .
                GetWilokeSubmission::renderPrice($aInfo['subTotal'], $aInfo['currency'], false) . ', ' .
                GetWilokeSubmission::renderPrice($aInfo['discount'], $aInfo['currency'], true) . ', ' .
                GetWilokeSubmission::renderPrice($aInfo['total'], $aInfo['currency'], false) . ', ' .
                $aInfo['gateway'] . ', ' . $aInfo['billingType'] . ', ' . $aInfo['created_at'] . "\n";
        }
    ...
    {
        add_submenu_page($this->parentSlug, esc_html__('Invoices', 'wiloke'), esc_html__('Invoices', 'wiloke'),
            'administrator', $this->slug, [$this, 'showInvoices']);
    }
    ...
    public function showInvoices()
    {
        ...
        ?>
        <div id="wil-invoice-table-wrapper" style="margin: 30px auto">
            <h2><?php esc_html_e('Invoices', 'wiloke'); ?></h2>
            <?php $this->getPartial('form-filters.php'); ?>

            <table id="wiloke-invoice-table" class="ui striped table">
                <?php
                ...
                <div class="ui segment">
                    <form action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" method="POST"
                        ...
                        <input type="hidden" name="args"
                               value="<?php echo esc_attr(urlencode(json_encode($_REQUEST))); ?>">
                        <input type="hidden" name="action" value="wiloke_submission_export_invoices">
                        <button class="js_export button ui basic purple"><?php esc_html_e('Export Payments',
                                'wiloke'); ?></button>
                        <button id="js_delete_invoices"
                                class="button ui basic red"><?php esc_html_e('Delete All Invoices',
                                'wiloke-listing-tools'); ?></button>
                    </form>
                </div>
            <?php endif; ?>
        </div>
