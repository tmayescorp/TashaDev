...
trait SetGeneral
{
    protected function getTimeZoneByGeocode()
    {
        ...
        if (!isset($this->aGoogleAddress['latLng']) || empty($this->aGoogleAddress['latLng'])) {
            return '';
        }
        
        if (\WilokeThemeOptions::getOptionDetail('map_type') == 'google_map') {
            $url       = 'https://maps.googleapis.com/maps/api/timezone/json?location='.$this->aGoogleAddress['latLng'].
                         '&timestamp='.time().'&key='.$wiloke->aThemeOptions['general_google_api'];
            ...
            $url          =
                'https://api.mapbox.com/v4/examples.4ze9z6tv/tilequery/'.$aParseLatLng[1].','.$aParseLatLng[0].
                '.json?access_token='.\WilokeThemeOptions::getOptionDetail('mapbox_api');
