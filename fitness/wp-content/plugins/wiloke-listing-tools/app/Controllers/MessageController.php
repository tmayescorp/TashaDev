...
class MessageController extends Controller
{
    ...
    public function __construct()
    {
        ...
        add_shortcode('wilcity_message_notification_shortcodes', [$this, 'renderMessageNotificationShortcode'], 15);
...
//        add_action('wp_head', [$this, 'inlineScripts'], 1);
    ...
    public function inlineScripts()
    {
        ...
        $count = MessageModel::countUnReadMessages(get_current_user_id());
        ?>
        <script>
            window.WIL_MESSAGES = {
    ...
    public function renderMessageNotificationShortcode($atts)
    {
        ob_start();
    ...
    public function countMessagesToMe()
    {
        ...
        if (empty($total)) {
            $oRetrieve->error([
                'msg' => esc_html__('No alerts or messages at this time', 'wiloke-listing-tools')
            ]);
        }
        $oRetrieve->success($total);
    }
    ...
    public function submitNewMsg()
    {
        ...
        $aStatus = $this->middleware(
            apply_filters(
                'wilcity/filter/submit-new-message-middleware',
                ['beforeSubmitMessage', 'isUserLoggedIn']
            ),
            apply_filters(
                'wilcity/filter/submit-new-message-middleware-options',
                [
        ...
        if (!Firebase::isFirebaseEnable()) {
            ...
            do_action(
                'wilcity/action/received-message',
                $_POST['receiveID'],
        ...
        if (!$msgID) {
            ...
            if (isset($_POST['isSentFromPopup'])) {
                $aResponse['instantMessage']
                    = esc_html__('Thank for contacting us, We will read and reply to you shortly',
                    'wiloke-listing-tools');
            }
    ...
    {
        $chatWith = absint($_POST['chattingWithId']);
        $userID = User::getCurrentUserID();
    ...
    private function getUserSkeleton($aUser)
    {
        ...
        $aAuthor['userID'] = absint($aUser['messageAuthorID']);
    ...
    public function fetchChattedWith()
    {
        ...
        $limit = isset($_POST['limit']) && !empty($_POST['limit']) ? absint($_POST['limit']) : 10;
