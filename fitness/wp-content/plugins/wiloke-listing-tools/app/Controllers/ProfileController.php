...
class ProfileController extends Controller
{
                ...
                case 'cover_image':
                    if (!empty($aField['value']) && isset($aField['value'][0])) {
                        if (isset($aField['value'][0]['id'])) {
                            $aUserMeta[sanitize_text_field($key)]
                                = wp_get_attachment_image_url($aField['value'][0]['id'], 'large');
                            ...
                            if (strpos($aField['value'][0]['src'], 'http') === false) {
                                ...
                                $instUploadImg->aData['uploadTo']
                                    = $instUploadImg::getUserUploadFolder($userID);
    ...
    public static function fetchProfileFields()
    {
        ...
        $aBasicInfo['avatar'] = [
            'value' => !empty($avatar) ?
                [['src' => $avatar, 'fileName' => esc_html__('Avatar', 'wiloke-listing-tools')]] : [],
            'type'  => 'upload_img',
            'label' => esc_html__('Avatar', 'wiloke-listing-tools')
        ];

        $coverImg = GetSettings::getUserMeta($userID, 'cover_image');
        $aBasicInfo['cover_image'] = [
            'value' => empty($coverImg) ? [] :
                [['src' => $coverImg, 'fileName' => esc_html__('Cover Image', 'wiloke-listing-tools')]],
            'type'  => 'upload_img',
        ...
        $aBasicInfo['description'] = [
            'value'       => get_the_author_meta('user_description', $userID),
            'type'        => 'textarea',
            'label'       => esc_html__('Introduce your self', 'wiloke-listing-tools'),
            'description' => esc_html__('Press Shift + Enter if you want to generate a new line.',
                'wiloke-listing-tools')
        ];

        $aBasicInfo['send_email_if_reply_message'] = [
        ...
        $aFollowContact['social_networks'] = [
        ...
        $aFollowContact
            = apply_filters('wilcity/wiloke-listing-tools/filter/profile-controllers/follow-contact', $aFollowContact);
