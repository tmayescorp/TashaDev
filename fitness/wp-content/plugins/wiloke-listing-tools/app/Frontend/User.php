...
class User
{
    public static    $aCoverImgs             = [];
    public static    $aAvatars               = [];
    public static    $aUsersSocialNetworks   = [];
    public static    $aUsersData             = [];
    public static    $aUsersPhone            = [];
    public static    $aUsersWebsite          = [];
    public static    $aUsersAddress          = [];
    public static    $aUsersPosition         = [];
    public static    $aUsersPicture          = [];
    public static    $aInstantMessages       = [];
    public static    $aAllMyPostIds          = [];
    public static    $aCountPosts            = [];
    public static    $aCanSubmitListingRoles = ['contributor', 'seller', 'vendor'];
    ...
    public static function isLockedAddListing()
    {
        $locked = GetSettings::getUserMeta(get_current_user_id(), 'locked_addlisting');

        return !empty($locked) ? $locked : false;
    }

    public static function getMyPosts($postType, $aQuery = [])
    {
        ...
        $query = new \WP_Query(
            apply_filters('wilcity/filter/wiloke-listing-tools/app/frontend/user/get-my-posts/args',
                $aArgs, $userID)
        );
    ...
    public static function getUserPass(WP_User $oUser): ?string
    {
        if (isset($oUser->data) && isset($oUser->data->user_pass) && !empty($oUser->data->user_pass)) {
        ...
        return $wpdb->get_var(
            $wpdb->prepare(
                "SELECT user_pass FROM $wpdb->users WHERE ID = %s LIMIT 1",
                $oUser->ID
            )
        );
    }
    ...
    {
        self::$userID = absint(self::$oPayLoad['userID']);
    ...
    public static function isSubmissionRole($userID)
    {
        ...
        foreach ($aRoles as $role) {
            if (in_array($role, apply_filters(
                'wilcity/wiloke-listing-tools/filter/wiloke-submission-roles',
                ['contributor', 'vendor', 'seller', 'administrator']
            ))) {
                return true;
            }
        }
    ...
    public static function currentUserCan($capability)
    {
        ...
        return call_user_func_array([$oUser, 'has_cap'], $args);
    }
    ...
    /**
     * @param string $post
     * @param bool $isAcceptAdmin
     *
     * @return bool
     */
    public static function isPostAuthor($post = null, $isAcceptAdmin = false)
    {
        if (empty($post)) {
            global $post;
            if (empty($post)) {
    ...
    public static function getFirstSuperAdmin()
    {
        ...
        $aArgs = apply_filters('wilcity/wiloke-listing-tools/get-first-super-admin', [
            'role'        => 'administrator',
            'orderby'     => 'ID',
            'total_users' => 1
        ]);
    ...
    public static function isUserConfirmedAccount($userID = null): bool
    {
        ...
        return current_user_can('administrator') ||
            GetSettings::getUserMeta($userID, 'confirmed') == 1;
    }

    public static function isAccountConfirmed($userID = null): bool
    {
        return self::isUserConfirmedAccount($userID);
    }

    public static function canSubmitListing($userID = '', $isFocusConfirm = true): bool
    {
    ...
    public static function countUserPosts($userID, $type, $perm = '')
    {
        ...
        $query
            = "SELECT post_status, COUNT( ID ) AS num_posts FROM {$wpdb->posts} WHERE post_type = %s AND post_author=%d AND post_status IN('" .
            implode("','", $aPostStatusKey) . "')";
    ...
    public static function getPosition($userID = null)
    {
        ...
        return apply_filters('wilcity/filter/wiloke-listing-tools/app/frontend/user/getPosition',
            self::$aUsersPosition[$userID]
        );
    }
    ...
    public static function getInstantMessage($userID = null)
    {
        ...
        self::$aInstantMessages[$userID] = empty(self::$aInstantMessages[$userID]) ?
            esc_html__(
                'Hi, thanks for your message. We are not here at the moment, but we\'ll get back to you soon!',
                'wiloke-listing-tools'
            ) : self::$aInstantMessages[$userID];
    ...
    public static function getAddress($userID = null)
    {
        ...
        return apply_filters('wilcity/filter/wiloke-listing-tools/app/frontend/user/getAddress',
            self::$aUsersAddress[$userID]
        );
    }
    ...
    public static function getPhone($userID = null)
    {
        ...
        if (function_exists('dokan_get_store_info')) {
            $aStoreSettings = dokan_get_store_info($userID);
            if (isset($aStoreSettings['phone']) && !empty($aStoreSettings['phone'])) {
                self::$aUsersPhone[$userID] = $aStoreSettings['phone'];

                return $aStoreSettings['phone'];
            }
        }

        self::$aUsersPhone[$userID] = GetSettings::getUserMeta($userID, 'phone');

        return apply_filters('wilcity/filter/wiloke-listing-tools/app/frontend/user/getPhone',
            self::$aUsersPhone[$userID]
        );
    }
    ...
    public static function getWebsite($userID = null)
    {
        ...
        return apply_filters('wilcity/filter/wiloke-listing-tools/app/frontend/user/getWebsite',
            self::$aUsersWebsite[$userID]
        );
    }
    ...
    public static function getSocialNetworks($userID = null)
    {
        ...
        if (empty($aSocialNetworks)) {
            self::$aUsersSocialNetworks[$userID] = [];
    ...
    public static function getField($field, $userID = null)
    {
        ...
        if ($field == 'user_pass') {
            return self::getUserPass($oUserData);
        }
    ...
    public static function renderFollower($authorID)
    {
        ...
        ?>
        <span id="<?php echo esc_attr(apply_filters(
            'wilcity/filter/id-prefix',
            'wilcity-follower-number-' . $authorID
        )); ?>" class="<?php echo esc_attr(apply_filters(
            'wilcity/filter/class-prefix',
            'wilcity-follower'
        )); ?> author-hero_rightText__1Yfm7"><span class="<?php echo esc_attr(apply_filters(
                'wilcity/filter/class-prefix',
                'wilcity-print-number'
            )); ?> color-primary"><?php echo esc_html(General::convertToNiceNumber($followers)); ?></span> <?php echo _n(
                'Follower',
                'Followers',
                $nFollowers,
                'wiloke-listing-tools'
    ...
    public static function renderFollowing($authorID)
    {
        ...
        ?>
        <span class="<?php echo esc_attr(apply_filters(
            'wilcity/filter/class-prefix',
            'wilcity-following'
        )); ?> author-hero_rightText__1Yfm7"><span
                class="color-primary"><?php echo esc_html(General::convertToNiceNumber($followings)); ?></span> <?php esc_html_e(
                'Following',
                'wiloke-listing-tools'
            ); ?></span>
        <?php
    ...
    public static function renderFollowStatus($authorID)
    {
        ...
        ?>
        <a class='color-primary fs-12 font-secondary font-bold'
           href='<?php echo esc_url(get_author_posts_url($authorID)); ?>'><i
                class='la la-refresh'></i> <?php echo FollowController::isIamFollowing($authorID) ?
                esc_html__('Following', 'wiloke-listing-tools') : esc_html__('Follow', 'wiloke-listing-tools'); ?></a>
        <?php
    }

    public static function getAllMyPostIds($isFocus = false, $aPostStatuses = ['publish'])
    {
        if (!empty(self::$aAllMyPostIds) && !$isFocus) {
            return self::$aAllMyPostIds;
        }

        $query = new \WP_Query(
            [
                'post_type'      => General::getPostTypeKeys(false, false),
                'post_status'    => $aPostStatuses,
                'posts_per_page' => 1000,
        ...
        while ($query->have_posts()) {
            $query->the_post();
            self::$aAllMyPostIds[] = $query->post->ID;
        }
