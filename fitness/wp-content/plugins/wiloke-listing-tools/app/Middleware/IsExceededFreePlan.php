...
//        }
        ...
        if (!isset($aOptions['isClaim']) && (isset($aOptions['listingID']) && !empty($aOptions['listingID']))) {
        ...
        $userID = $aOptions['userID'] ?? User::getCurrentUserID();

        $totalListingsSubmitted = PlanRelationshipModel::countListingsUserSubmittedInPlan($aOptions['planID'], $userID);
        if ($totalListingsSubmitted >= absint($aPlanSettings['availability_items'])) {
            $this->msg =
                esc_html__('Oops! You exceeded the number of free listings of this plan', 'wiloke-listing-tools');
