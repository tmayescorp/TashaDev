...
class QRCodeGenerator
{
    ...
    public static function generateTicket($orderID)
    {
        ...
        if (!class_exists('\QRcode')) {
            include plugin_dir_path(__FILE__) . 'phpqrcode/phpqrcode.php';
        }
        ...
        foreach ($aProducts as $productID) {
            if (self::isSendQRCodeToEmail($productID)) {
                ...
                if (!empty($oOrder->get_billing_phone())) {
                    $aContents['phone'] = sprintf(esc_html__('Tel: %s', 'wiloke-listing-tools'),
                        $oOrder->get_billing_phone());
                }
                ...
                $aContents['productName'] = sprintf(esc_html__('Product name: %s', 'wiloke-listing-tools'),
                    get_the_title($productID));
                $aContents['purchasedDate'] = sprintf(esc_html__('Purchased date: %s', 'wiloke-listing-tools'),
                    $purchasedDate);
                $aContents['purchasedFrom'] = sprintf(esc_html__('Purchased from: %s', 'wiloke-listing-tools'),
                    $purchasedFrom);

                $aContents = apply_filters('wilcity/qrcode/my_products/content', $aContents, $productID, $orderID,
                    $oOrder);
