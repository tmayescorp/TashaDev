...
class HTML
{
    ...
    {
        $aAtts['wrapperClass'] =
            !empty($aAtts['marginClass']) ? $aAtts['wrapperClass'].' '.$aAtts['marginClass'] : $aAtts['wrapperClass'];

        return $aAtts;
    }

    public static function generateLabelClasses($aAtts, $defaultClasses = 'field_label__2eCP7 text-ellipsis')
    {
        $labelClasses = isset($aAtts['isRequired']) && $aAtts['isRequired'] === 'yes' ?
            $defaultClasses.' required' : $defaultClasses;

        return $labelClasses;
    }

    public static function renderDynamicField($aAtts)
    {
        switch ($aAtts['type']) {
            case 'text':
            case 'input':
            case 'wil-input':
                $aAtts['type'] = isset($aAtts['inputType']) ? $aAtts['inputType'] : 'text';
                self::renderInputField($aAtts);
                break;
            case 'select':
            case 'wil-select':
                $aAtts['type'] = 'text';
                self::renderSelectField($aAtts);
                break;
            case 'textarea':
            case 'wil-textarea':
                self::renderTextareaField($aAtts);
                break;
            case 'checkbox':
            case 'wil-checkbox':
                self::renderCheckboxField($aAtts);
                break;
            case 'hidden':
            case 'wil-hidden':
                self::renderHiddenField($aAtts);
                break;
        }
    ...
    public static function renderCheckboxField($aAtts)
    {
        ...
        <div class="<?php echo esc_attr($aAtts['wrapperClass']); ?>">
            <label class="<?php echo esc_attr(self::generateLabelClasses($aAtts, 'checkbox_label__3cO9k')); ?>">
                <input class="checkbox_inputcheck__1_X9Z" type="checkbox" name="<?php echo esc_attr($aAtts['name']); ?>"
                       value="<?php echo esc_attr(isset($aAtts['trueValue']) ? $aAtts['trueValue'] : $aAtts['value']); ?>"/>
                ...
                <?php if (!in_array($aAtts['name'], ['isAgreeToTermsAndConditionals', 'isAgreeToPrivacyPolicy'])) : ?>
                    <span class="checkbox_text__3Go1u text-ellipsis">
                        <?php \Wiloke::ksesHTML($aAtts['label']); ?>
                        <span class="checkbox-border"></span>
                    </span>
                <?php endif; ?>
            </label>
            <?php if (in_array($aAtts['name'], ['isAgreeToTermsAndConditionals', 'isAgreeToPrivacyPolicy'])) : ?>
                <span class="checkbox_text__3Go1u text-ellipsis"><?php \Wiloke::ksesHTML($aAtts['label']); ?>
                    <span class="checkbox-border"></span></span>
            <?php endif; ?>
    ...
    public static function renderHiddenField($aAtts)
    {
        ...
        ?>
        <input type="hidden" name="<?php echo esc_attr($aAtts['name']); ?>"
               value="<?php echo esc_attr($aAtts['value']); ?>"/>
        <?php
    }
    ...
    public static function renderInputField($aAtts)
    {
        ...
        if (!in_array($aAtts['type'], ['email', 'password', 'text'])) {
            return '';
        }
        $aAtts = self::mergeClass($aAtts);
        ?>
        <div class="<?php echo esc_attr($aAtts['wrapperClass']); ?>">
            <div class="<?php echo esc_attr($aAtts['innerClass']); ?>">
                <input name="<?php echo esc_attr($aAtts['name']); ?>"
                       class="<?php echo esc_attr($aAtts['inputClass']); ?>"
                       value="<?php echo esc_attr($aAtts['value']); ?>" type="<?php echo esc_attr($aAtts['type']); ?>">
                <span
                    class="<?php echo esc_attr(self::generateLabelClasses($aAtts)); ?>"><?php echo esc_html($aAtts['label']); ?></span>
    ...
    public static function renderTextareaField($aAtts)
    {
        $aAtts = wp_parse_args(
            $aAtts,
            [
        ...
        <div class="<?php echo esc_attr($aAtts['wrapperClass']); ?>">
            <div class="<?php echo esc_attr($aAtts['innerClass']); ?>">
                <textarea name="<?php echo esc_attr($aAtts['name']); ?>"
                          class="<?php echo esc_attr($aAtts['inputClass']); ?>"><?php echo esc_textarea($aAtts['value']);
                    ?></textarea>
                <span
                    class="<?php echo esc_attr(self::generateLabelClasses($aAtts)); ?>"><?php echo esc_html($aAtts['label']); ?></span>
                <span class="bg-color-primary"></span>
            ...
            <div class="<?php echo esc_attr($aAtts['innerClass']); ?>">
                ...
                <div class="field_right__2qM90 pos-a-center-right">
                    <a href="#" class="wil-btn wil-btn--primary wil-btn--round wil-btn--xxs js-upload-single-image">
                        ...
                        <div class="pill-loading_module__3LZ6v" style="display: none;">
    ...
    public static function renderSelectField($aAtts)
    {
        ...
        <div class="<?php echo esc_attr($aAtts['wrapperClass']); ?>">
            <div class="<?php echo esc_attr($aAtts['innerClass']); ?>">
                <select name="<?php echo esc_attr($aAtts['name']); ?>"
                        class="<?php echo esc_attr($aAtts['inputClass']); ?>">
                    <?php if (!empty($aAtts['options']) && is_array($aAtts['options'])) : ?>
                        <?php foreach ($aAtts['options'] as $val => $name) : ?>
                            <option value="<?php echo esc_attr($val) ?>" <?php selected($aAtts['value'],
                                $val); ?>><?php echo esc_html($name) ?></option>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </select>
    ...
    public static function renderSiteLogo()
    {
        ...
        if (empty($logo)) {
            $logo =
                isset($wiloke->aThemeOptions['general_logo']['url']) ? $wiloke->aThemeOptions['general_logo']['url'] :
                    '';
        }

        if (empty($logo2x)) {
            $logo2x = isset($wiloke->aThemeOptions['general_retina_logo']['url']) ?
                $wiloke->aThemeOptions['general_retina_logo']['url'] : '';
        }
        ?>
        <a class="<?php echo esc_attr(apply_filters('wilcity/filter/id-prefix', 'wilcity-site-logo')); ?>"
           href="<?php echo esc_url(home_url('/')); ?>">
            <?php if (empty($logo2x)) : ?>
                <img src="<?php echo esc_url($logo); ?>" alt="<?php bloginfo('name'); ?>"/>
            <?php else: ?>
                <img src="<?php echo esc_url($logo); ?>" srcset="<?php echo esc_url($logo2x) ?> 2x"
    ...
    public static function renderLink(
        $class = '', $name = '', $link = '', $icon = '', $linkID = '', $includePreloader = false)
    {
        $class  = 'wil-btn '.$class;
        $linkID = empty($linkID) ? uniqid('link_id') : $linkID;
        ?>
        <a id="<?php echo esc_attr($linkID); ?>" class="<?php echo esc_attr(trim($class)); ?>"
           href="<?php echo esc_url($link); ?>"><?php if (!empty($icon)): ?><i
                class="<?php echo esc_attr($icon); ?>"></i><?php endif; ?><?php echo esc_html($name); ?> <?php if ($includePreloader): ?>
                <div class="pill-loading_module__3LZ6v hidden">
                    <div class="pill-loading_loader__3LOnT"></div>
                </div><?php endif; ?></a>
        <?php
    }
    ...
    public static function renderTable($aColumnTitles, $aColumnValues, $tblClass = '')
    {
        ...
        <table class="<?php echo esc_attr($tblClass); ?>">
            <thead>
            <tr>
                <?php foreach ($aColumnTitles as $aColumn) : ?>
                    <th><?php echo esc_html($aColumn['name']); ?></th>
                <?php endforeach; ?>
            </tr>
            </thead>
            <tbody>
            <tr>
                <?php foreach ($aColumnValues as $key => $value) : ?>
                    <td data-th="<?php echo esc_attr($aColumnTitles[$key]['name']); ?>"
                        class="<?php echo esc_attr($aColumnTitles[$key]['class']); ?>"><?php echo esc_html($value); ?></td>
                <?php endforeach; ?>
            </tr>
            </tbody>
        </table>
    ...
    public static function renderPaymentButtons()
    {
        ...
        $aPaymentData = apply_filters('wilcity/wiloke-listing-tools/renderPaymentButtons', [
            'paypal'       => [
        ...
        foreach ($aGateways as $gateway):
            ...
            <div class="<?php echo esc_attr($itemClass); ?>">
                ...
                <div
                    class="icon-box-2_module__AWd3Y wil-text-center <?php echo esc_attr($aPaymentData[$gateway]['bg']); ?>">
                    <a id="wilcity-proceed-with-<?php echo esc_attr($gateway); ?>" data-gateway="<?php echo esc_attr
                    ($gateway); ?>"
                       class="disable wilcity-gateway-box" href="#">
