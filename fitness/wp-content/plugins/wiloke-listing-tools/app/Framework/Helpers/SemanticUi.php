...
class SemanticUi {
	...
	public static function getPosts($aField){
	    ...
	    if (isset($aField['meta_query'])) {
	        $Args['meta_query'] = $aField['meta_query'];
	    }

	    if(WPML::isActive()){
            $Args = WPML::addFilterLanguagePostArgs($Args);
	    }
            ...
            <div data-query="<?php echo esc_attr($aField['post_type']); ?>">
                <select id="<?php echo esc_attr($aField['id']); ?>" <?php echo esc_attr($required); ?> class="<?php echo esc_attr($aField['class']); ?> js_select2_ajax" name="<?php echo esc_attr($aField['name']); ?>" <?php echo esc_attr($multiple); ?>>
					...
					if ( !empty($aField['value']) ){
						$aField['value'] = is_array($aField['value']) ? $aField['value'] : [$aField['value']];
    ...
    public static function renderSelectTwoField($aField){
		$wrapperClasses = 'field' . (isset($aField['wrapperClasses']) ? ' ' . $aField['wrapperClasses'] : '');
		$classes = (isset($aField['classes']) ? ' ' . $aField['classes'] : '');
		$required = isset($aField['required']) && $aField['required'] ? 'required' : '';

		$attributes = '';
		if (isset($aField['dataAtts']) && is_array($aField['dataAtts'])) {
            foreach ($aField['dataAtts'] as $key => $val) {
                $attributes .= $key .'="'.$val.'"';
            }
		}
		?>
        <div class="<?php echo esc_attr($wrapperClasses); ?>">
            <label for="<?php echo esc_attr($aField['id']); ?>"><?php echo esc_attr($aField['heading']); ?></label>
            <select name="<?php echo esc_attr($aField['name']); ?>" <?php echo esc_attr($required); ?>
                    id="<?php echo esc_attr($aField['id']); ?>"
                    <?php echo $attributes; ?>
    ...
    /**
     *
     * @param array $aField $aField = ['heading' => 'Heading', 'id' => 'required', 'class' => 'additional']
    */
    public static function renderButton(array $aField)
    {
		?>
		<div class="field">
