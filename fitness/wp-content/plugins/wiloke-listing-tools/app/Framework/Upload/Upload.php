...
'image/png'
    ...
    private function insertAttachment($file, $fileType, $uploadTo, $postId = 0)
    {
    ...
    public function uploadFakeFile()
    {
        if ($this->aData['aFile']['size'] > self::fileSize()) {
            return sprintf(esc_html__('You can not upload a file bigger than %s', 'wiloke-listing-tools'),
        ...
        $uploadOverride = ['test_form' => false];
    ...
    public function image($postId = 0)
    {
        ...
        return $this->insertAttachment($aFileReturn['file'], $aFileReturn['type'], $uploadTo . $fileName, $postId);
    }
}
